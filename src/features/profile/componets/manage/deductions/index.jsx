...
import useGetPenalties, { useGetDeductions } from './hook/useGetDeductions';
import { useTranslation } from 'react-i18next';



export default function Deductions({id}) {
  const idUser =id
  ...
  const { mutate:getDeductionsDate, data: DataWarnings } = useGetDeductions();
...
<StackRow >
  <Box>
 <TypoHeader sx={{color:"#131627",fontWeight:"500"}}>{t('Deductions')} </TypoHeader>
  </Box>
  <Box>
...
<StackRow >
 ...
 <TypoHeader>{t('Total')}  {DataWarnings?.data?.data?.my_decisions?.length} {t('Deductions')} </TypoHeader>
  </Box>
...
<>
      ...
      <Typography  sx={{fontWeight:"600",fontSize:"16px",color:"#131627"}}>{t('Delete')} </Typography>
        ...
        {t("Are you sure you want to delete")+t("Deductions")+"?"}
      </DialogTitle>
